...
for w in ['distance','uniform']:
  for k in range(1,30):
    neigh.set_params(n_neighbors=k,weights=w)
    neigh.fit(data_train,label_train)
    predictions=neigh.predict(data_dev)
    ...
    if this_score>score:
        ...
        set_K=k
        set_weights=w
neigh.set_params(n_neighbors=set_K,weights=set_weights)
print("---------Best Model----------")
...
test_res=neigh.predict_proba(test_list)
